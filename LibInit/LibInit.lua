...
---@diagnostic disable-next-line: undefined-field
local off = (_G.RED_FONT_COLOR_CODE or '|cffff0000') ..
	(_G.VIDEO_OPTIONS_DISABLED or 'Off') .. (_G.FONT_COLOR_CODE_CLOSE or '|r')
---@diagnostic disable-next-line: undefined-field
local on = (_G.GREEN_FONT_COLOR_CODE or '|cff00ff00') ..
	(_G.VIDEO_OPTIONS_ENABLED or 'On') .. (_G.FONT_COLOR_CODE_CLOSE or '|r')
...
local function ParseDebugStack(stack)
	...
	local regexp =
	"Interface[\\/]AddOns[\\/]([^\\/]*)[\\/]([%w_\\.\\/]+)[^:]+:(%d+):" -- modified to allow for dots in the file name
...
function lib:Debug(...)
	if self.debug then
		...
		local header = format("|cffff9900%s|r (%s:%d)", tostring(self), ParseDebugStack(debugstack(2, 1, 0)))
...
function lib:OnLeaveCombat(action, ...)
	...
	if type(action) == "string" and type(self[action]) ~= "function" then
		error(
			"Usage: OnLeaveCombat (\"action\", ...): 'action' - method '" .. tostring(action) .. "' not found on self.",
			2)
	end
...
function lib:GetChatFrame(chat)
	if (chat) then
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			if (frame.name == chat) then
				lib.chats[chat] = frame
				return frame
			end
		end
		return nil
	...
	for _, section in ipairs(HELPSECTIONS) do
		if (section == RELNOTES) then
			self:HF_Load(section, main .. section,
				' ' .. tostring(self.version) .. ' (r:' .. tostring(self.revision) .. ')')
		else
			self:HF_Load(section, main .. section, '')
		end
	end
...
-- @section coroutine
lib.coroutines = lib.coroutines or setmetatable({}, {
...
function lib:coroutineOnEnd(signature, action, ...)
	...
	if type(action) == "string" and type(self[action]) ~= "function" then
		error("Usage: OnCoroutineEnd (\"action\", ...): 'action' - method '" .. tostring(action) ..
			"' not found on self.", 2)
	end
	if type(action) == "string" then
...
-- Debug output system. Code originally from LibDebug bt
local reserved =
{
	["and"] = true,
	["break"] = true,
...
local function LibInit_LibDebugLinkHandler(link, text, linkData, contextData)
	local id = tonumber(linkData and linkData.options)
	if id then
		ShowLibDebugTableById(id)
		return LinkProcessorResponse.Handled
	end
end

if LinkUtil and LinkUtil.RegisterLinkHandler and not lib.libdebugLinkHandlerRegistered then
	if LinkUtil.IsLinkHandlerRegistered and LinkUtil.IsLinkHandlerRegistered(LIBINIT_LINK_TYPE) then
...
function prettyPrint(header, ...)
	...
	temp[1] = header
	for i = 1, n do
		temp[i] = myToString(select(i, ...))
	end

	AceConsole:Print(header, table.concat(temp, " "))
	del(temp)
end
